...
class MiCuenta extends Page
{
    ...
    public function mount(): void
    {
        ...
        // Guardar la URL de referencia si no existe ya en la sesión
        if (!session()->has('mi_cuenta_referrer')) {
            $referrer = request()->headers->get('referer');
            if ($referrer && !str_contains($referrer, 'mi-cuenta')) {
                session(['mi_cuenta_referrer' => $referrer]);
            } else {
                // Si no hay referrer válido, usar home como fallback
                session(['mi_cuenta_referrer' => route('filament.admin.pages.home')]);
            }
        }
    }
    ...
    public function siguientePaso(): void
    {
        ...
        if ($this->pasoActual == 1) {
            $this->validate([
                ...
                'datosEdicion.celular' => ['required', 'regex:/^(\d{9}|\d{11})$/'],
    ...
    public function volverPaginaAnterior()
    {
        $referrerUrl = session('mi_cuenta_referrer', route('filament.admin.pages.home'));
        
        // Limpiar la sesión
        session()->forget('mi_cuenta_referrer');
        
        // Redirigir a la página anterior
        return redirect($referrerUrl);
    }

    // Método para cerrar y redirigir después de editar
